...
__global__ void multiplyKernelDoubleBuffering(float *d_A, float *d_B, float *d_C, int m, int k, int n) {
    ...
    __shared__ float tileA[2][TILE_WIDTH][TILE_WIDTH + 2];
    __shared__ float tileB[2][TILE_WIDTH][TILE_WIDTH + 2]; // changing the padding to + 2 for float2 vectorization
    ...
    for (int w = 0; w < (k + TILE_WIDTH - 1) / TILE_WIDTH; w++) {
        ...
        for (int k_inst = 0; k_inst < TILE_WIDTH; k_inst++) {
            ...
            // Can't use float4 to load fragA here since we are grabbing the k_inst column of tileA to fragA, which is not continous space in memory (unless we transpose A in loading to tile phase)
            #pragma unroll
            for (int i = 0 ; i < THREAD_TILE_H; i++) {
            ...
            // Try vectorized fragB loading since we are grabbing the k_inst row of tileB
            // Need to use float2 instead of float4 since THREAD_TILE_W = 2
            float2* fragBPtr = reinterpret_cast<float2*>(&tileB[curr_phase][k_inst][tx * THREAD_TILE_W]);
            float2 fragB_vec = *fragBPtr;
            ...
            for (int i = 0 ; i < THREAD_TILE_H; i++) {
                ...
                // Vectorized fragB
                accum[i][0] += fragA[i] * fragB_vec.x;
                accum[i][1] += fragA[i] * fragB_vec.y;
            }
        }
