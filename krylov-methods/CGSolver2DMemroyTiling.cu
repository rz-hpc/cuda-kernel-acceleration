...
__global__ void spmv_tiled_5pt_kernel(int width,
                                      int height,
                                      const float* d_p,
                                      float* d_q,
                                      float* d_partial_product) {

    // shared tile memory (16 + 2) * (16 + 2)
    // 16 x 16 threads in the tile, + 2 for the boundary one row and one column
    //__shared__ float s_p[TILE_H + 2][TILE_W + 2];
    __shared__ float s_p[TILE_H + 2][TILE_W + 2 + 1]; // for bank conflicts

    // local -> global index mapping: s_p[ty + 1][tx + 1] = d_p[gy * width + gx]
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    int gx = blockIdx.x * TILE_W + tx;
    int gy = blockIdx.y * TILE_H + ty;

    // Each thread (16x16) gets the center value
    if (gx < width && gy < height) {
        s_p[ty + 1][tx + 1] = d_p[gy * width + gx];
    }
    else {
        s_p[ty + 1][tx + 1] = 0.0f;
    }

    // Bounday thread (x, y 0 or 15) fill the boundaries
    // left halo
    if (tx == 0) {
        s_p[ty + 1][0] = (gx > 0 && gy < height) ? d_p[gy * width + gx - 1] : 0.0f;
    }

    // right halo
    if (tx == TILE_W - 1) {
        s_p[ty + 1][TILE_W + 1] = (gx < width - 1 && gy < height) ? d_p[gy * width + gx + 1] : 0.0f;
    }

    // top halo
    if (ty == 0) {
        s_p[0][tx + 1] = (gy > 0 && gx < width) ? d_p[(gy - 1) * width + gx] : 0.0f;
    }

    // bottom halo
    if (ty == TILE_H - 1) {
        s_p[TILE_H + 1][tx + 1] = (gy < height - 1 && gx < width) ? d_p[(gy + 1) * width + gx] : 0.0f;
    }

    // critical: wait for all threads finish loading values
    __syncthreads();

    // Calculation the q and p_dot_q (partial)
    if (gx < width && gy < height) {
        float north = s_p[ty + 1 - 1][tx + 1];
        float west = s_p[ty + 1][tx + 1 - 1];
        float center = s_p[ty + 1][tx + 1];
        float east = s_p[ty + 1][tx + 1 + 1];
        float south = s_p[ty + 1 + 1][tx + 1];

        // 2d poisson 4.0 center -1.0 neighbors
        float val = 4.0 * center - (north + west + east + south);

        // update the d_q
        int row_idx = gy * width + gx;
        d_q[row_idx] = val;

        // Update the partial product, d_p is still in register/cache
        d_partial_product[row_idx] = center * val;
    }
...
float solveCG(int n, int nnz, int* d_ptr, int* d_cols, float* d_vals, float* d_b, float* d_x, const int max_iter, const float tolerance) {
    ...
    for(int k=0; k < max_iter; k++) {
        ...
        */
        ...
        dim3 blocks_tiled((grid_dim + 15) / 16, (grid_dim + 15) / 16);
        spmv_tiled_5pt_kernel<<<blocks_tiled, threads_tiled>>>(grid_dim, grid_dim, d_p, d_q, d_partial_product);
...
int main() {
    ...
    int max_iter = 10000;// set to small number for NSight profiling 10000;
