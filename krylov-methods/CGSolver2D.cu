...
// Combined x and r update together to save memory trip
// x += x * p, r -= r * q
...
void generatePoisson2D(const int grid_size,
    ...
    int n = grid_size * grid_size; // total nodes
    ...
    for (int i = 0; i < grid_size; i++) {
        for (int j = 0; j < grid_size; j++) {
            // every "Point" on your plate needs its own unique ID.
            // We call this ID the row_idx because it represents one specific row (one equation) in our massive Matrix $A$.
            // row_idx means: "The ID of the point I am currently writing an equation for."
            int row_idx = i * grid_size + j;

            // The row_ptr tells us where the math for Point 4 (row_idx) starts.
            // The col_indices tell us that Point 4 is linked to Point 3, Point 5, Point 1, and Point 7 (col_indices).

            // For math, the neighbor order doesn't matter, but for GPU memory coelased
            // If your column indices are sorted from smallest to largest within each row,
            // the GPU can sometimes fetch the corresponding values from the vector $x$ more efficiently.
            // the index from smallest to largest are North -> West -> Center -> East -> South

            // Neighbor North
            if (i > 0) {
                //col_indices.push_back( (i-1) * grid_size + j);
            ...
            // Neighbor West
            if (j > 0) {
                //col_indices.push_back( i * grid_size + j - 1);
            ...
            // Center
            //col_indices.push_back( i * grid_size + j);
            ...
            // This needs to be inside the j loop
            // A row in matrix A represent one node, which id is represented by i * grid_size + j
