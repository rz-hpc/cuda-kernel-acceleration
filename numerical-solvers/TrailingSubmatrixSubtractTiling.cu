...
// Step 3: trailing matrix subtract with tiling (GEMM/SYRK)
// A22 = L21 L21^T + L22 L22^T -> A22` = A22 - L21 L21^T
// Tile: load L21 to shared memory to reduce global memory access
__global__ void trailing_submatrix_kernel(float* d_A, int n, int k_block) {

    // shared memory: one for row L21, one for col L21
    __shared__ float tile_row[BLOCK_DIM][BLOCK_DIM + 1];
    __shared__ float tile_col[BLOCK_DIM][BLOCK_DIM + 1];
    ...
    // A22 target final write back row and col
    int global_row_idx = target_block_row * BLOCK_DIM + ty;
    int global_col_idx = target_block_col * BLOCK_DIM + tx;

    // L21 panel global row
    // load L21 panel row (target_block_row, k_block) for Multiplication
    int row_panel_global_row = target_block_row * BLOCK_DIM + ty;
    int row_panel_global_col = k_block * BLOCK_DIM + tx;

    // L21 panel global col
    // load L21 panel col (target_block_col, k_block) for Multiplication
    int col_panel_global_row = target_block_col * BLOCK_DIM + ty;
    int col_panel_global_col = k_block * BLOCK_DIM + tx;

    // Load L21 row and column to the shared memory
    if (row_panel_global_row < n && row_panel_global_col < n) {
        tile_row[ty][tx] = d_A[row_panel_global_row * n + row_panel_global_col];
    }
    else {
        tile_row[ty][tx] = 0.0f;
    }
    if (col_panel_global_row < n && col_panel_global_col < n) {
        tile_col[ty][tx] = d_A[col_panel_global_row * n + col_panel_global_col];
    }
    else {
        tile_col[ty][tx] = 0.0f;
    }
    ...
    // Compute block local dot product (for each column k in the block)
    float sum = 0.0f;
    for (int k = 0; k < BLOCK_DIM; k++) {
        sum += tile_row[ty][k] * tile_col[tx][k];
    }

    // In-place global matrix subtract (lower triangle and diagonal)
    if (global_row_idx < n && global_col_idx < n) {
        if (target_block_row > target_block_col || (target_block_row == target_block_col && ty >= tx)) {
            d_A[global_row_idx * n + global_col_idx] -= sum;
        }
    }
}
...
int main() {
    ...
    for (int k_block = 0; k_block < total_blocks; k_block++) {
        ...
        if (blocks_below > 0) {
            ...
            trailing_submatrix_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_A, N, k_block);
            CHECK_CUDA(cudaGetLastError());
            CHECK_CUDA(cudaDeviceSynchronize());
        }
