...
__host__ void linearSolverHost(float* A, float* x, float* b, int n) {
  ...
  dim3 BlocksPerGridLU2((n + 32 - 1) / 32, (n + 16 - 1) / 16, 1);
...
int main() {
    ...
    for (int i = 0; i < n; i++) h_b[i] = (float)rand() / (float)RAND_MAX;
    ...
    for (int i = 0; i < n; i++) {
        ...
        double error = fabs(row_sum - h_b[i]);
        total_error += error;
    ...
    free(h_A); free(h_b); free(h_x); free(h_A_copy);
    return 0;
}
