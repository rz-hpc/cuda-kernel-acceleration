...
__global__ void tsqr_merge_optimized_kernel(float* d_A, float* d_tau, float* d_V_merge, int M, int N, int stride, int lda, int level, int initial_blocks) {
    ...
    for (int k = 0; k < N; k++) {
        ...
        // update trailing columns
        // A = A - tau * v * (v^T * A)
        // with warp shuffling optimization
        // all the threads in one warp work on one column together
        for (int j = k + 1; j < N; j ++) {
            // All rows from k to M_merge-1, strided across threads
            // tile_v[k]=1, tile_v[k+1..N-1]=0, tile_v[N..2N-1]=stored vecs
            // No special-casing needed — zeros handle the upper-tri rows automatically
            float dot = 0.0f;
            for (int i = k + tx; i < M_merge; i += blockDim.x) {
                dot += tile_v[i] * tile_A[i][j];
            }

            // warp-level reduction: aggregate each thread's dot
            for (int offset = 16; offset >= 1; offset >>= 1) {
                dot += __shfl_down_sync(0xffffffff, dot, offset);
            }

            // Broadcast the final result, thread 0 saves it
            dot = __shfl_sync(0xffffffff, dot, 0);
...
int main() {
    ...
    qr_base_optimized_kernel<<<initial_blocks, BLOCK_DIM>>>(d_A, d_tau, BLOCK_ROWS, N, lda);
    ...
    while (active_blocks > 1) {
        ...
        tsqr_merge_optimized_kernel<<<next_level_blocks, BLOCK_DIM>>>(d_A, d_tau, d_V_merge, M, N, stride, lda, level, initial_blocks);
        cudaDeviceSynchronize();
