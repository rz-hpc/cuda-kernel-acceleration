...
__global__ void panel_qr_baseline(float* d_A, float* d_tau, int M, int N, int lda) {
    ...
    __shared__ float s_alpha; // first element of target vector
    __shared__ float s_v_first; // first element of reflection vector
    ...
    // for each column in this panel
    for (int k = 0; k < N; k++) {
        // calculate norm, first element of reflect vector, and tau (thread 0)
        if (tx == 0) {
        ...
        // populate the reflect vector v (gride-stride)
        for (int i = tx; i < M; i += blockDim.x) {
            if (i == k) {
                ...
                tile_v[i] = 0.0f; // clean out upper triangle
            }
        }
        __syncthreads();

        // save R's diagonal value
        if (tx == 0) {
            tile_A[k][k] = s_alpha;
        }
        __syncthreads();

        // update trailing columns (to the right of k)
        // H1 A = A - tau * v1 * (v1^T A)
        // j: gride-stride loop k + 1 + tx, j += blockDim.x
        for (int j = k + 1 + tx; j < N; j += blockDim.x) {
            // calculate dot product of v1^t A
            float dot = 0.0f;
            for (int i = k; i < M; i++) {
        ...
        // save reflection coefficients to global memory
        if (tx == 0) {
            d_tau[k] = s_tau;
        }
    }
