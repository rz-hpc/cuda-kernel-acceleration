...
- **`numerical-solvers/`**
  ...
  * **Blocked Cholesky Factorization:** High-performance dense $A = LL^T$ decomposition. Features a cooperative, shared-memory tiled trailing submatrix update kernel optimized for implicit transposition layouts that bypasses global memory bandwidth constraints to achieve **~79.25% SM Compute Throughput** and a **38.4× execution time speedup** on the dominant kernel. Detailed Nsight hardware metrics analysis is archived in [`cholesky_profiling_report.md`](https://github.com/rz-hpc/cuda-kernel-acceleration/blob/main/numerical-solvers/cholesky_profiling_report.md).
  * **Tall-Skinny QR (TSQR) Factorization:** High-performance, numerically stable QR decomposition for tall-skinny matrices ($M \gg N$). Implements a hierarchical tree-reduction architecture: leaf blocks run independent Householder panel factorizations, and a recursive merge kernel stacks pairs of $R$ factors for further reduction. A decoupled `d_V_merge` workspace prevents Householder vector clobbering across tree levels. The warp-shuffle optimized panel kernel reassigns all 32 warp threads to cooperate on each trailing column via `__shfl_down_sync` dot-product reduction, achieving a **2.21× speedup** (67.14 µs → 30.37 µs) and near-doubling warp lane utilization (8.90 → 16.76 active threads/warp). Detailed Nsight hardware metrics analysis is archived in [`tsqr_profiling_report.md`](https://github.com/rz-hpc/cuda-kernel-acceleration/blob/main/numerical-solvers/tsqr_profiling_report.md).
...
- **Execution Profiling Breakdown:**
  * **Naive Baseline:** 0.228608 ms (1.0× baseline) — limited by uncoalesced, strided global memory writes across column indices separated by $N$ elements.
  * **Shared Memory Tiled (32×32):** 0.120672 ms (~1.9× speedup) — restructures global access into coalesced horizontal reads and writes via `__syncthreads()` block memory barriers.
  * **Tiled + Bank Padded (32×33):** 0.073728 ms (~3.1× speedup) — resolves internal shared memory bank conflicts by altering row-stride mapping parameters.
...
### 4. Blocked Cholesky Factorization — Shared Memory Tiled Trailing Submatrix Update

- **Hardware Diagnostic Target:** Eliminating global memory round-trips in the $A_{22} = A_{22} - L_{21}L_{21}^T$ trailing update by staging panel tiles in shared memory and exploiting implicit transposition for conflict-free broadcast access.
- **Profiling Configuration:** $N = 2048$, $32 \times 32$ thread blocks, 64 outer loop iterations, NVIDIA Tesla T4 (sm\_75).
- **Nsight Compute Diagnostics (Trailing Update Kernel — Kernel 3):**

| Metric | Naive (`cholesky_native`) | Tiled (`cholesky_tiled`) | Delta |
|---|---|---|---|
| FP32 Roofline Achievement | ~1.0% | ~8–9% | **8–9× compute peak utilization** |
| SM Active Cycles | 3,357,342 | 80,183 | **41.9× reduction** |
| Duration (first wave) | 5.82 ms | 151.74 µs | **38.4× speedup** |
| Compute (SM) Throughput | 6.43% | 79.25% | **12.3× increase** |
| Memory Throughput | 49.44% | 79.25% | Balanced execution profile |
| DRAM Throughput | 1.18% | 11.39% | Cache hits reducing DRAM pressure |
| L1/TEX Cache Throughput | 98.88% | 87.86% | Shared memory paths replace L1 pressure |
| IPC Active | 0.07 | ~0.92 | **13.1× instruction density improvement** |
| Warp Cycles Per Issued Instruction | 486.28 | 33.44 | **14.5× scheduling turnaround** |

- **Micro-Architectural Bottleneck Breakdown & Analysis:**
  * **Naive bottleneck — the memory wall:** Every inner loop iteration fetches directly from global DRAM (`d_A[global_row * n + dot_idx]`). The L1/LG instruction queue becomes saturated with in-flight VRAM transactions, stalling the warp scheduler for **473.6 of every 485.7 cycles** between instruction issues (97.5% stall rate). IPC collapses to 0.07.
  * **Tiled optimization — constant broadcast from shared memory:** Loading the panel block into `tile_row` and `tile_col` shared memory tiles eliminates all VRAM traffic during the dot product phase. Because `tx` is fixed across threads in the same column lane, every thread in the warp requests the identical `tile_row[ty][k]` address simultaneously — the hardware intercepts this as a **constant broadcast**, serving all 32 threads in a single shared memory cycle with zero bank conflicts.
  * **Remaining bottleneck — MIO pipeline saturation:** With DRAM stalls eliminated, the tight accumulation loop (`sum += tile_row[ty][k] * tile_col[tx][k]`) saturates the Memory Input/Output (MIO) shared memory dispatch queue. Stall cycles drop from 473.6 to 22.7 per instruction issue, but this queue saturation now accounts for 67.9% of remaining stalls — the characteristic signature of a kernel that has graduated from memory-bound to instruction-latency-bound.


### 5. Tall-Skinny QR (TSQR) — Warp-Shuffle Panel Factorization
...
- **Profiling Configuration:** $M = 512$, $N = 4$, 4 leaf blocks × 128 rows, 3-level reduction tree, NVIDIA Tesla T4 (sm\_75).
- **Nsight Compute Diagnostics (Panel QR Kernel):**

| Metric | Baseline (`panel_qr_baseline_kernel`) | Optimized (`qr_base_optimized_kernel`) | Delta |
|---|---|---|---|
...
| Avg. Active Threads / Warp | 8.90 | 16.76 | **+88% warp lane utilization** |
| Warp Cycles Per Issued Instruction | 6.01 | 5.68 | 5.5% reduction |
| Compute (SM) Throughput | 1.32% | 0.94% | Memory-bound regime |

- **Micro-Architectural Bottleneck Breakdown & Analysis:**
  * **Baseline bottleneck — warp underutilization:** The baseline trailing update assigned one thread per trailing column (`j = k + 1 + tx`). With $N = 4$, at most 3 threads were active during the compute phase per Householder step; 29 of 32 warp lanes were idle on every instruction issue slot. The 8.90 average active threads per warp — under one full quarter of the warp — confirms this directly.
  * **Optimization — column-per-warp with warp shuffle:** The optimized kernel pivots the inner loop to `i = k + tx; i < M; i += blockDim.x`, partitioning the 128-row dot product across all 32 threads (4 rows each). A `__shfl_down_sync` butterfly reduction aggregates partial sums into thread 0 without touching shared memory. Active threads rise to 16.76 per warp — utilization nearly doubled.
  * **Merge kernel:** The `tsqr_merge_optimized_kernel` shows increased active threads (8.81 → 12.48/warp) but flat wall-clock time. With $M_{\text{merge}} = 2N = 8$ rows and BLOCK\_DIM = 32, 24 of 32 threads contribute zero to every dot product — an architectural floor at $N = 4$. The design is correct and will scale favorably at $N \geq 16$.
  * **Remaining bottleneck — serial norm reduction:** Thread 0 still computes $\sum x_i^2$ serially over all $M = 128$ rows at the start of each column step (~40–45% fixed-latency stall). A parallel shared-memory tree reduction across all 32 threads is the next optimization target.
